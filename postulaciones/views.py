...
from .forms import PostulacionForm, PostulacionStatusUpdateForm
...
def marcar_notificaciones_leidas(request):
    if request.method == 'POST' and hasattr(request.user, 'estudianteprofile'):
        estudiante = request.user.estudianteprofile
        estudiante.notificaciones.filter(leida=False).update(leida=True)
    ...
    def form_valid(self, form):
        ...
        if prev_status != new_status:
            ...
            if new_status == 'Aceptada':
                mensaje = f'Tu postulación a "{form.instance.vacante.titulo}" ha sido aceptada.'
            elif new_status == 'Rechazada':
                mensaje = f'Tu postulación a "{form.instance.vacante.titulo}" ha sido rechazada.'
            elif new_status == 'Entrevista':
                mensaje = f'Tu postulación a "{form.instance.vacante.titulo}" está en entrevista.'
